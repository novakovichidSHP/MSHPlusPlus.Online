...
// Инлайн-ввод в консоли — без подсказок-плейсхолдеров (терминальный стиль).
const CONSOLE_INPUT_PLACEHOLDER_DESKTOP = "";
const CONSOLE_INPUT_PLACEHOLDER_MOBILE = "";
...
function bindUi() {
  ...
  // Поле растёт под длину ввода (инлайн-каретка в потоке вывода).
  els.consoleInput.addEventListener("input", autoSizeConsoleInput);
  autoSizeConsoleInput();
  // Клик по консоли фокусирует поле ввода (как в терминале), не мешая выделению текста.
  els.consoleOutput.addEventListener("click", (event) => {
    if (event.target === els.consoleInput) return;
  ...
  document.addEventListener("keydown", (event) => {
    ...
    // Хоткеи сравниваем по event.code (физическая клавиша), а не event.key:
    // на русской раскладке event.key для Alt+R = "к" и т.п. → комбинации не ловились.
    // Run code (F8 / Alt+R)
    if (event.key === "F8" || (event.altKey && event.code === "KeyR")) {
      event.preventDefault();
      runActiveFile();
    }
    // Stop execution (Alt+X)
    ...
    if (event.altKey && (event.code === "Digit1" || event.code === "Numpad1")) {
      event.preventDefault();
      if (state.editorAdapter) {
    ...
    if (event.altKey && (event.code === "Digit2" || event.code === "Numpad2")) {
      event.preventDefault();
      els.consoleInput.focus();
    }
  });
...
function applyEditorSettings() {
  ...
  if (els.editorWrap) {
    ...
    // Высота строки тоже должна следовать за размером шрифта, иначе при A-/A+
    // меняется только ширина глифов, а число видимых строк не растёт.
    els.editorWrap.style.setProperty("--code-line-height", `${Math.round(fontSize * 1.5)}px`);
  }
  if (els.editor) {
...
function clearConsole() {
  // Поле инлайн-ввода живёт ВНУТРИ потока консоли (всегда последним) — при очистке
  // удаляем только вывод и возвращаем поле в конец.
  const input = els.consoleInput;
  els.consoleOutput.textContent = "";
  if (input) els.consoleOutput.appendChild(input);
  state.outputBytes = 0;
}

/**
 * Вставляет узел в поток консоли ПЕРЕД полем инлайн-ввода, чтобы каретка ввода
 * всегда оставалась в самом конце вывода (как в настоящем терминале).
 */
function consoleInsert(node) {
  const anchor = els.consoleInput;
  if (anchor && anchor.parentNode === els.consoleOutput) {
    els.consoleOutput.insertBefore(node, anchor);
  } else {
    els.consoleOutput.appendChild(node);
  }
}

/** Метка сегмента консоли (КОМПИЛЯЦИЯ / ВЫВОД ПРОГРАММЫ) — как в макете. */
function appendConsoleLabel(text, variant) {
  const label = document.createElement("div");
  label.className = variant ? `seg-label ${variant}` : "seg-label";
...
function appendConsoleStyled(text, className) {
  ...
  consoleInsert(span);
  consoleInsert(document.createElement("br"));
  els.consoleOutput.scrollTop = els.consoleOutput.scrollHeight;
}
...
function appendConsole(text, isError) {
  ...
  if (state.outputBytes > CONFIG.MAX_OUTPUT_BYTES) {
    consoleInsert(document.createTextNode("\n[вывод обрезан]\n"));
    return;
  }
  if (isError) {
    const span = document.createElement("span");
    ...
    appendConsoleText(frag, normalized);
    consoleInsert(frag);
  }
  els.consoleOutput.scrollTop = els.consoleOutput.scrollHeight;
}
...
function sendConsoleLine() {
  ...
  // Эхо набранного — тем же цветом, что и вывод, прямо в потоке (как терминал):
  // текст «фиксируется» в строке промпта, затем перевод строки (нажатый Enter).
  consoleInsert(document.createTextNode(line));
  consoleInsert(document.createElement("br"));
...
/** Ширина инлайн-поля = длине ввода (моноширинный шрифт → 1 символ = 1ch). */
...
async function runActiveFile() {
  ...
  appendConsoleLabel("Вывод программы", "ok");
  ...
  if (runResult.timedOut) {
    ...
    appendConsoleStyled(`\nПрограмма завершена с кодом ${runResult.exitCode ?? 0}`,
